# ✍️ Meta-Prompt Mestre: Arquitetura de Informação, DevOps, Rclone, Mattermost, GitHub Issues, Git Graph & Governança

Este é o **Prompt Mestre Definitivo**. Ele reúne as diretrizes de **Arquitetura de Informação, Redação Técnica (Modelo Híbrido), Governança DevOps, Backup 3-2-1 com Rclone & Google Drive, Alertas no Mattermost, Automação Idempotente de GitHub Issues via GitHub Actions, Visualização Gráfica de Branches (Git Graph / Mermaid) e a criação dos 8 arquivos de documentação padrão**.
